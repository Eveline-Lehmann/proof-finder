#-------------------------------------------------------
# Programm to test derivability of formulas in IL
#
...
""" Function that checks, whether some sequent is derivable within ghpc (see comment above).
    ...
    Then the right-Implication rule is applied if possible. Since this rule is not invertible
    a backtracking is given at hand. Finally, if no other rules than branching ones are applicable
    the branching formulas are collected and one of them is chosen randomly. If at the end not even
    a branching rule is applicable it remains to test, whether the final sequent is an axiom
    and hence the original formula is derivable.
...
def derive(left, right, history, recorded=False, current_node=None):
    ...
    if isAxiom(left, right):
        ...
        """ The following lines treats those rules, that either need a branching or that are not invertable
            which then needs a possibility to bachtrack. Backtracking comes before branching. """
...
def isAxiom(left, right):
    """ Returns True if left -> right is an axiom."""
