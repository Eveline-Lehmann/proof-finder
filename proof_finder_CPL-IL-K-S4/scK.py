#-------------------------------------------------------
# Programm to test derivability of modal formulas in scK
#
...
""" Function that checks, whether some formula is derivable within scK (as presented by Hererding Alain, Sequent Calculi
    for Proof Search in Some Modal Logics, PhD thesis University of Bern, Definition 5.2.2). It uses a two-sided sequent calculi
...
def derivable(formula, recorded=False, in_nnf = False):
    ...
    if recorded and not in_nnf:
        ...
        n2.set_rule('nnf')
        current_node = n2
    return derive(left, right, recorded, current_node)
...
def derive(left, right, recorded=False, current_node=None):
    ...
    if isAxiom(right):
        ...
        for formula in copy_right:
            if formula[1]=='*':
                ...
                if recorded:
                    ...
                    n1.set_rule('$\land$')
                    n2.set_rule('$\land$')
                    answer1 = derive(lleft, lright, recorded, n1)
                    answer2 = derive(rleft, rright, recorded, n2)
                else:
                    answer1 = derive(lleft, lright)
                    answer2 = derive(rleft, rright)                    
        ...
        while idx < number_of_boxes:
            copy_left=left[:]
            l, r  = ruleBox(copy_left,right[idx])
            idx += 1
            n = None
            if recorded:
                s = Sequent()
                ...
                n = Node(s)
                current_node.add_child(n)
            is_derivable = derive(l,r, recorded, n)
            if is_derivable:
                if recorded:
                    n.set_rule('$\Box$')
                return True
...
def ruleBox(left, fright):
    """ Only use if fright = box g for some g. From dia f1... dia fn -> fright... box gm to empty -> f1,...fn,fright."""
